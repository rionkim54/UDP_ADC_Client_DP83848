...
void udp_echoclient_sendEx(char * pdata)
{
  struct pbuf *p;

  /* allocate pbuf from pool*/
  p = pbuf_alloc(PBUF_TRANSPORT,strlen((char*)pdata), PBUF_POOL);
  
  if (p != NULL)
  {
    /* copy data to pbuf */
    pbuf_take(p, (char*)pdata, strlen((char*)pdata));
...
void udp_receive_callback(void *arg, struct udp_pcb *pcb, struct pbuf *p,const ip_addr_t *addr, u16_t port)
{
  ...
  message_count++;
	...
	// printf("udp_receive_callback %d\r\n", message_count);
	printf("udp_receive_callback %s\r\n", p->payload);
